...
async def chatbot():
    print(
        "Greg: Hi! I am here to help you find interesting courses for next semester. What are you interested in?"
    )
    while True:
        user_input = input("> ")
        if user_input.lower() in ["exit", "quit"]:
        ...
        # Use GPT to understand and generate a response based on the user's input
        # processed_input = get_gpt_response(user_input)
        print(f"Greg: Let me check that for you...")
        description = generate_hypothetical_description(user_input)
        courses = get_related_courses(description)
        chatbot_response = summarize_related_courses(courses, user_input)
        print(f"Greg: {chatbot_response}")
